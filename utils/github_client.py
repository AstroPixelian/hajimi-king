...
class GitHubClient:
    ...
    def search_for_keys(self, query: str, max_retries: int = 5) -> Dict[str, Any]:
        ...
        for page in range(1, 11):
            ...
            (
                page_result,
                (page_total_requests, page_failed_requests, page_rate_limit_hits),
            ) = self.search_by_page(query, page=page, page_size=page_size)
        ...
        result = {
            ...
            "incomplete_results": final_count < expected_total
            if expected_total
            else False,
            "items": all_items,
        }
    ...
    def search_by_page(
        self, query: str, page: int, page_size=100, max_retries: int = 5
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                if proxies:
                    ...
                    response = requests.get(
                        self.GITHUB_API_URL, headers=headers, params=params, timeout=30
                    )
                logger.info(
                    f"Requesting page {page} for query: {query[:50]}...{response}"
                )
                rate_limit_remaining = response.headers.get("X-RateLimit-Remaining")
                # 只在剩余次数很少时警告
                if rate_limit_remaining and int(rate_limit_remaining) < 3:
                    logger.warning(
                        f"⚠️ Rate limit low: {rate_limit_remaining} remaining, token: {current_token}"
                    )
                ...
                if status in (403, 429):
                    ...
                    if attempt == max_retries:
                        logger.error(
                            f"❌ HTTP {status} error after {max_retries} attempts on page {page}"
                        )
        ...
        try:
            ...
            if proxies:
                metadata_response = requests.get(
                    metadata_url, headers=headers, proxies=proxies
                )
            else:
            ...
            if encoding == "base64" and content:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Failed to decode base64 content: {e}, falling back to download_url"
                    )
            ...
            if proxies:
                content_response = requests.get(
                    download_url, headers=headers, proxies=proxies
                )
            else:
                content_response = requests.get(download_url, headers=headers)
            logger.info(
                f"⏳ checking for keys from:  {download_url},status: {content_response.status_code}"
            )
            ...
            logger.error(
                f"❌ Failed to fetch file content: {metadata_url}, {type(e).__name__}"
            )
            return None
